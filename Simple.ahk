...
Loop_GemRuns()
{
	...
	while (!gFound_Error)
	{
		...
		; selects and runs adventure logic
		...
		; resetes if anventure window is found
		bAdventureWindowFound := AdventureWindow_Check(1)
		if (bAdventureWindowFound)
		{
			ResetAdventure()
...
; Handle Start/Transition/Reset
{
	RunAdventure()
	{
		...
		; checks is current level is higher than desired level
		; ends run if true
		while (bContinueRun)
		{
			; DoLEvel must return false here for some reason
